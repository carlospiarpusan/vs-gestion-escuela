...
const emptyMForm = {
  vehiculo_id: "",
  instructor_id: "",
  tipo: "gasolina" as TipoMantenimiento,
  descripcion: "",
  monto: "",
  kilometraje_actual: "",
  litros: "",
...
function safeFloat(v: string, fb = 0) {
...
function safeFloatOrNull(v: string) {
  const n = parseFloat(v);
  return isNaN(n) ? null : n;
}
...
export default function VehiculosPage() {
  ...
  const [, setVehiculosTotalCount] = useState(0);
  const [vehiculosCurrentPage] = useState(0);
  const [vehiculosSearchTerm] = useState("");
  const [loadingV, setLoadingV] = useState(true);
  const [, setTableErrorV] = useState("");
  ...
  const [, setMantenimientosTotalCount] = useState(0);
  const [mantenimientosCurrentPage] = useState(0);
  const [mantenimientosSearchTerm] = useState("");
  const [loadingM, setLoadingM] = useState(true);
  const [, setTableErrorM] = useState("");
  ...
  const [, setHistorialLoading] = useState(false);
  const [, setHistorialError] = useState("");
  ...
  const fetchVehiculosTable = useCallback(
    async (page = 0, search = "") => {
      ...
      try {
        ...
        setTableErrorV(
          fetchError instanceof Error ? fetchError.message : "No se pudieron cargar los vehículos."
        );
      } finally {
  ...
  const fetchBitacoraTable = useCallback(
    async (page = 0, search = "") => {
      ...
      try {
        ...
        setTableErrorM(
          fetchError instanceof Error ? fetchError.message : "No se pudo cargar la bitácora."
        );
      } finally {
  ...
  useEffect(() => {
    ...
    perfil?.escuela_id,
    tab,
    mantenimientosCurrentPage,
    mantenimientosSearchTerm,
  ]);
  ...
  const openEditV = (row: Vehiculo) => {
    ...
    setFormV({
      ...
      fecha_itv: row.fecha_itv || "",
      seguro_vencimiento: row.seguro_vencimiento || "",
      estado: row.estado,
      kilometraje: row.kilometraje.toString(),
      notas: row.notas || "",
    });
    setErrorV("");
    setModalVOpen(true);
  ...
  const handleSaveV = async () => {
    if (!formV.marca || !formV.modelo || !formV.matricula) {
      setErrorV("Marca, modelo y matrícula son obligatorios.");
      return;
    }
    setSavingV(true);
    setErrorV("");
    try {
      ...
      const payload = {
        marca: formV.marca,
        modelo: formV.modelo,
        matricula: formV.matricula,
        tipo: formV.tipo,
        anio: formV.anio ? parseInt(formV.anio) : null,
        fecha_itv: formV.fecha_itv || null,
        seguro_vencimiento: formV.seguro_vencimiento || null,
        estado: formV.estado,
        kilometraje: parseInt(formV.kilometraje) || 0,
      ...
      setErrorV(e instanceof Error ? e.message : "Error al guardar");
      setSavingV(false);
    }
  };
  ...
  const handleDeleteV = async () => {
    ...
    try {
      ...
      setErrorV(e instanceof Error ? e.message : "Error al eliminar");
      setSavingV(false);
    }
  };
  ...
  const openCreateM = () => {
    ...
    restoreMantenimientoDraft({
      ...emptyMForm,
      instructor_id: isInstructor ? (currentInstructorId ?? "") : "",
    });
    setErrorM("");
    setModalMOpen(true);
  };
  const openEditM = (row: MantenimientoVehiculo) => {
    ...
    setFormM({
      ...
      kilometraje_actual: row.kilometraje_actual?.toString() || "",
      litros: row.litros?.toString() || "",
      precio_por_litro: row.precio_por_litro?.toString() || "",
  ...
  const syncVehiculoKilometraje = useCallback(
    async (vehiculoId: string, kilometrajeActual: number | null) => {
  ...
  const handleSaveM = async () => {
    if (!formM.vehiculo_id || !formM.descripcion) {
      setErrorM("Vehículo y descripción son obligatorios.");
      return;
    }
    setSavingM(true);
    setErrorM("");
    try {
      ...
      const payload = {
        ...
        proveedor: formM.proveedor || null,
        numero_factura: formM.numero_factura || null,
        fecha: formM.fecha,
        notas: formM.notas || null,
      };
      if (editingM) {
        await runSupabaseMutationWithRetry(() =>
      ...
      setErrorM(e instanceof Error ? e.message : "Error al guardar");
      setSavingM(false);
    }
  };
  ...
  const handleDeleteM = async () => {
    ...
    try {
      ...
      setErrorM(e instanceof Error ? e.message : "Error al eliminar");
      setSavingM(false);
    }
  };
  ...
  const colsV = [
    ...
    {
      key: "kilometraje" as keyof Vehiculo,
      label: "Km",
      render: (r: Vehiculo) => <span>{r.kilometraje.toLocaleString()} km</span>,
    },
    {
      key: "estado" as keyof Vehiculo,
      label: "Estado",
      render: (r: Vehiculo) => (
        ...
        <div className="space-y-1">
          ...
          {r.ultimo_registro_fecha ? (
            <p className="text-xs leading-5 text-[#86868b]">
              Último: {formatTipoMantenimiento(r.ultimo_registro_tipo || "")} el{" "}
              {formatFecha(r.ultimo_registro_fecha)}
  ...
  const colsM = [
    ...
    {
      key: "vehiculo_nombre" as string,
      label: "Vehículo",
      render: (r: MantenimientoVehiculo & { vehiculo_nombre?: string }) => (
        <span className="font-medium">{r.vehiculo_nombre}</span>
      ),
    },
    {
      key: "tipo" as keyof MantenimientoVehiculo,
      label: "Tipo",
      render: (r: MantenimientoVehiculo) => (
        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${tipoColors[r.tipo]}`}>
          {formatTipoMantenimiento(r.tipo)}
        </span>
      ),
    },
    { key: "descripcion" as keyof MantenimientoVehiculo, label: "Descripción" },
    {
      key: "monto" as keyof MantenimientoVehiculo,
      label: "Monto",
      render: (r: MantenimientoVehiculo) => (
    ...
    {
      key: "kilometraje_actual" as keyof MantenimientoVehiculo,
      label: "Km",
      render: (r: MantenimientoVehiculo) => (
        <span>{r.kilometraje_actual ? `${r.kilometraje_actual.toLocaleString()} km` : "—"}</span>
      ),
    },
  ];
  ...
  return (
    <div>
      ...
      <Modal
        ...
        title={editingV ? "Editar Vehículo" : "Nuevo Vehículo"}
        maxWidth="max-w-xl"
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                type="text"
                value={formV.marca}
                onChange={(e) => setFormV({ ...formV, marca: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={formV.modelo}
                onChange={(e) => setFormV({ ...formV, modelo: e.target.value })}
                className={inputCls}
              />
            </div>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              ...
              <input
                type="text"
                value={formV.matricula}
                onChange={(e) => setFormV({ ...formV, matricula: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              ...
              <select
                value={formV.tipo}
                onChange={(e) => setFormV({ ...formV, tipo: e.target.value as TipoVehiculo })}
                className={inputCls}
              >
                {tipos.map((t) => (
            ...
            <div>
              ...
              <input
                type="number"
                value={formV.anio}
                onChange={(e) => setFormV({ ...formV, anio: e.target.value })}
                className={inputCls}
              />
            </div>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              ...
              <input
                type="number"
                value={formV.kilometraje}
                onChange={(e) => setFormV({ ...formV, kilometraje: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              <label className={labelCls}>Fecha ITV</label>
              <input
                type="date"
                value={formV.fecha_itv}
                onChange={(e) => setFormV({ ...formV, fecha_itv: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              <label className={labelCls}>Vence Seguro</label>
              <input
                type="date"
                value={formV.seguro_vencimiento}
                onChange={(e) => setFormV({ ...formV, seguro_vencimiento: e.target.value })}
                className={inputCls}
              />
            </div>
          </div>
          <div>
            ...
            <select
              value={formV.estado}
              onChange={(e) => setFormV({ ...formV, estado: e.target.value as EstadoVehiculo })}
              className={inputCls}
            >
              {estadosV.map((e) => (
          ...
          <div>
            ...
            <textarea
              value={formV.notas}
              onChange={(e) => setFormV({ ...formV, notas: e.target.value })}
              rows={2}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {savingV ? "Guardando..." : editingV ? "Guardar Cambios" : "Crear Vehículo"}
            </button>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        open={modalMOpen}
        onClose={() => setModalMOpen(false)}
        title={editingM ? "Editar Registro de Bitácora" : "Nuevo Registro de Bitácora"}
        maxWidth="max-w-xl"
      >
        <div className="space-y-4">
          {errorM && (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                value={formM.vehiculo_id}
                onChange={(e) => setFormM({ ...formM, vehiculo_id: e.target.value })}
                className={inputCls}
              >
            ...
            {isInstructor ? (
              ...
              <div>
                ...
                <select
                  value={formM.instructor_id}
                  onChange={(e) => setFormM({ ...formM, instructor_id: e.target.value })}
                  className={inputCls}
                >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                value={formM.tipo}
                onChange={(e) => setFormM({ ...formM, tipo: e.target.value as TipoMantenimiento })}
                className={inputCls}
              >
                {tiposMant.map((t) => (
                  <option key={t} value={t}>
                    {formatTipoMantenimiento(t)}
                  </option>
                ))}
              </select>
            </div>
            <div>
              ...
              <input
                type="date"
                value={formM.fecha}
                onChange={(e) => setFormM({ ...formM, fecha: e.target.value })}
                className={inputCls}
              />
            </div>
          </div>
          <div>
            ...
            <input
              type="text"
              value={formM.descripcion}
              onChange={(e) => setFormM({ ...formM, descripcion: e.target.value })}
              className={inputCls}
            />
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              ...
              <input
                ...
                step="0.01"
                value={formM.monto}
                onChange={(e) => setFormM({ ...formM, monto: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              ...
              <input
                type="number"
                value={formM.kilometraje_actual}
                onChange={(e) => setFormM({ ...formM, kilometraje_actual: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              ...
              <input
                ...
                step="0.01"
                value={formM.litros}
                onChange={(e) => setFormM({ ...formM, litros: e.target.value })}
                className={inputCls}
              />
            </div>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              <label className={labelCls}>$/Litro</label>
              <input
                type="number"
                step="0.01"
                value={formM.precio_por_litro}
                onChange={(e) => setFormM({ ...formM, precio_por_litro: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={formM.proveedor}
                onChange={(e) => setFormM({ ...formM, proveedor: e.target.value })}
                className={inputCls}
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={formM.numero_factura}
                onChange={(e) => setFormM({ ...formM, numero_factura: e.target.value })}
                className={inputCls}
              />
            </div>
          </div>
          <div>
            ...
            <textarea
              value={formM.notas}
              onChange={(e) => setFormM({ ...formM, notas: e.target.value })}
              rows={2}
              className={`${inputCls} resize-none`}
            />
          </div>
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              className="rounded-lg bg-[#0071e3] px-4 py-2 text-sm text-white transition-colors hover:bg-[#0077ED] disabled:opacity-50"
            >
              {savingM ? "Guardando..." : editingM ? "Guardar Cambios" : "Crear Registro"}
            </button>
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        onClose={() => {
          ...
          setHistorialVehiculoId(null);
        }}
        title={
          historialVehiculo
            ? `Bitácora de ${historialVehiculo.marca} ${historialVehiculo.modelo}`
        ...
        {historialVehiculo && (
          <div className="space-y-5">
            <div className="apple-panel-muted grid gap-3 px-4 py-4 sm:grid-cols-3">
              <div>
                <p className="text-[11px] font-semibold tracking-[0.18em] text-[#86868b] uppercase">
                  Vehículo
                </p>
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-[#1d1d1f] dark:text-[#f5f5f7]">
                  {(historialVehiculo as VehiculoConBitacora).registros_bitacora || 0} registro
                  {(historialVehiculo as VehiculoConBitacora).registros_bitacora === 1 ? "" : "s"}
                </p>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${tipoColors[registro.tipo]}`}
                          >
                          ...
                          {registro.kilometraje_actual != null && (
                            <span className="text-xs text-[#86868b]">
                              {registro.kilometraje_actual.toLocaleString()} km
                            </span>
                          )}
                        </div>
                        ...
                        <p className="mt-1 text-xs leading-5 text-[#86868b]">
                          ...
                          {registro.precio_por_litro != null
                            ? ` · ${formatCop(registro.precio_por_litro)} / litro`
                            : ""}
      ...
      <DeleteConfirm
        ...
        onConfirm={handleDeleteV}
        loading={savingV}
        error={errorV}
        message={`¿Eliminar ${deletingV?.marca} ${deletingV?.modelo} (${deletingV?.matricula})?`}
      />
      <DeleteConfirm
        open={deleteMOpen}
        onClose={() => setDeleteMOpen(false)}
        onConfirm={handleDeleteM}
        loading={savingM}
        error={errorM}
        message="¿Eliminar este registro de bitácora?"
      />
    </div>
  );
}
